...
to run:
python gee_subset.py -p "COPERNICUS/S1_GRD" -b "HH" -f "D:\Krishna\projects\vwc_from_radar\data\fuel_moisture\nfmd_queried_latlon_subset.csv" -pd 0.05 -d "D:\Krishna\projects\vwc_from_radar\data\sar\new"

python gee_subset.py -p COPERNICUS/S1_GRD -b VH -l 48.09888889	-120.1425 -pd 0.05 -s 2018-01-01 -e 2018-01-30
...
#        for (start, end) in split_years(args.start, args.end):
        ...
        if df.shape == (0,0):
            ...
            if args.directory:
                if not(os.path.isdir(os.path.join(args.directory, args.bands[0]))):
                    os.mkdir(os.path.join(args.directory, args.bands[0])) 
                df.to_csv(os.path.join(args.directory,args.bands[0],"%s_gee.csv"%loc[1]), index = False)  
                print('[INFO]\t'
                      '[{0}/{1}]\t'
                      'Time Taken {2}\t'
